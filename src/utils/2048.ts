...
export const moveMapIn2048Rule = (
  ...
  const { result, isMoved, scoreToAdd } = moveLeft(rotatedMap);
...
const moveLeft = (map: Map2048): MoveResult => {
  ...
  const scoreToAdd = movedRows.reduce((sum, row) => sum + row.scoreToAdd, 0);
  return { result, isMoved, scoreToAdd };
};

const moveRowLeft = (
  row: Cell[]
): { result: Cell[]; isMoved: boolean; scoreToAdd: number } => {
  const reduced = row.reduce(
    (acc: { lastCell: Cell; result: Cell[]; scoreToAdd: number }, cell) => {
      if (cell === null) {
        return acc;
        ...
        return {
          ...
          lastCell: null,
          scoreToAdd: acc.scoreToAdd + cell * 2,
        };
      } else {
        return {
          result: [...acc.result, acc.lastCell],
          lastCell: cell,
          scoreToAdd: acc.scoreToAdd,
        };
      }
    },
    { lastCell: null, result: [], scoreToAdd: 0 }
  );

  const result = [...reduced.result, reduced.lastCell];
  const scoreToAdd = reduced.scoreToAdd;
...
type MoveResult = { result: Map2048; isMoved: boolean; scoreToAdd: number };
